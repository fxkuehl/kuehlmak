...
The `-p ranking` option causes it to save the ranked layout to files `ranking01.kbl` ... `ranking10.kbl`. This makes it easy to file your favourite layouts away for reference or edit them for minor adjustments and run them through `kuehlmak eval` again.
...
2. Run `kuehlmak anneal` for more iterations and/or lower `-s` setting to generate a wider variety of layouts. Try `-s1000 -n200` or even `-s100 -n1000` to get a much larger variety. Then use `kuehlmak rank` with the criteria that matter to you most.
3. Change the weights of layout stats in the configuration file and redo the annealing and ranking steps.
4. Set targets in the configuration file to emphasize some stats while avoiding over-optimizing at the expense of other objectives and redo the annealing and ranking steps.
...
To analyze keyboard layouts, you need a text corpus for your language. If there is no existing one, you can create one from a (preferably large) plain text file in your language. The `kuehlmak corpus` sub-command can turn any text file into a JSON file with statistics about letter, bigram and 3-gram counts suitable for the layout analyzer.
...
Try setting target values for the scores that you care most about. Let the other stats sort themselves out with their weights alone. If you want to force the optimizer to get closer to the targets, try increasing the `factor`. Values between 1 and 5 may be sensible, 2 is a good starting point. An example that works well for me:
...
At the top left there is a total score that is what `kuehlmak anneal` will try to minimize. The second number is based on constraints set in the configuration file. In this example it is a measure of divergence from QWERTY. Below that you find the detailed metrics that summarize the strengths and weaknesses of each layout.
...
These are uncomfortable sequences of three same-hand keys. This includes two bad bigrams (SFBs, scissor or lateral stretches) chained together (e.g. "kno"), disjointed Scissors or dSFBs with another key on the same hand between them (e.g. "car"), as well as same-finger 3-grams (e.g. "nny").
...
The starting point for the estimate uses is the letter frequency data and the distance of each key from the home position. The return to the home position is not counted because that is just the result of relaxing the finger. This estimate can be corrected with same-finger bigrams and skipgrams by subtracting the travel distance from home to the last key and adding the distance from the first to the last key.
